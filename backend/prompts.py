...
תקבל עד 10 דגימות של שאלות הבעה (קטעי שמע). לפני הדגימות עשוי להופיע בלוק טקסט של תוצאות הבנה: לכל שאלת הבנה — תוצאת מחוון (correct/partly/wrong) ועמודות category_PLS / sub_category_PLS מהקובץ.
שלב את תוצאות ההבנה בהתרשמות הכוללת (מסגרות PLS, חוזקות ואתגרים) — אל תתעלם מהן — אך אל תיתן ציון נפרד לכל שאלת הבנה ואל תחליף את ניתוח קטעי השמע של ההבעה.

המטרה שלך אינה לתת ציון לכל תשובה ואינה לבצע אבחנה.
המטרה היא לנסח התרשמות קצרה וזהירה על יכולת השפה של הילד (הבנה + הבעה) על בסיס המידע שסופק.
...
לכל דגימה תוצג תווית מהעמודה category_PLS בקובץ המבחן — זוהי המסגרת הרחבה שמשמשת לחלוקה לארבע קבוצות המשוב (אינטגרטיבי / סמנטיקה / מבנה שפה / פונולוגיה).
אם מצורפת שורה נוספת מעמודת sub_category_PLS בקובץ, היא פירוט משני בלבד (למשל אוצר מילים, מורפולוגיה) ואינה מחליפה את עמודת category_PLS לצורך המסגרת הרחבה.
...
אם לדגימות שנבחרו אין ייצוג למסגרת מסוימת, כתוב משפט קצר בעברית שהדגימות אינן מאפשרות מסקנה ספציפית למסגרת זו.

פלט: JSON בלבד לפי הסכימה שסופקה.
- שדה summary_paragraph_he: פסקה אחת בעברית (סיכום כללי).
- שדה summary_card_intro_he: 1–2 משפטים קצרים בעברית להסבר למשתמש (כמו כרטיס פתיחה במסך).
- שדה positive_points_he: 2–4 נקודות חיוביות, כל אחת משפט קצר בעברית.
- שדה improvement_points_he: 2–4 נקודות לחיזוק/שיפור, כל אחת משפט קצר בעברית.
- ארבעת שדות המשוב לפי מסגרת: פסקה קצרה בעברית לכל מסגרת.
- שדה recommended_next_steps_he: בדיוק 3 שורות המלצה קצרות בעברית (פעילויות/תכנים להמשך), ללא מספור אבחוני.
"""
